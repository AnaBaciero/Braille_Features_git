...
RawDataSet1 <- list.files(pattern = "xP32_") %>%
  lapply(read_csv) %>% 
  bind_rows%>%
  mutate(Subject= rep(list.files(pattern = "xP32_"), each=331))%>%
...
RawDataSet2 <- list.files(pattern = "yP32_") %>%
  lapply(read_csv) %>% 
  bind_rows%>%
  mutate(Subject= rep(list.files(pattern = "yP32_"), each=329))%>%
  separate(`IT1 IT2 KEY RT`,c("IT1","a", "IT2","b","c","d","e","f","g","KEY", "RT"), sep = " ")%>%
  select(Subject,IT1,IT2,KEY,RT)%>%
  filter(IT1 != "S",
         IT1 !="DESCANSO")%>%
...
P32VaRaw<-RawDataSet1%>%
  bind_rows(RawDataSet2)

save(P32VaRaw, file = "P32VaRaw.RData")
