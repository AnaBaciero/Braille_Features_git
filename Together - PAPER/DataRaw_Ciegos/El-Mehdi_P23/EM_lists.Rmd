...
write_delim(L1,"L1_EM.txt", col_names = F, delim = "")

L2 <- Start%>%
  bind_rows(Practice2,List2)%>%
  as_data_frame()

write_delim(L2,"L2_EM.txt", col_names = F, delim = "")

L3 <- Start%>%
  bind_rows(Practice3,List3)%>%
  as_data_frame()

write_delim(L3,"L3_EM.txt", col_names = F, delim = "")

L4 <- Start%>%
  bind_rows(Practice4,List4)%>%
  as_data_frame()

write_delim(L4,"L4_EM.txt", col_names = F, delim = "")
```
