...
RawDataSet1 <- list.files(pattern = "xP31_") %>%
  lapply(read_csv) %>% 
  bind_rows%>%
  mutate(Subject= rep(list.files(pattern = "xP31_"), each=331))%>%
...
RawDataSet2 <- list.files(pattern = "yP31_") %>%
  lapply(read_csv) %>% 
  bind_rows%>%
  mutate(Subject= rep(list.files(pattern = "yP31_"), each=329))%>%
  separate(`IT1 IT2 KEY RT`,c("IT1","a", "IT2","b","c","d","e","f","g","KEY", "RT"), sep = " ")%>%
  select(Subject,IT1,IT2,KEY,RT)%>%
  filter(IT1 != "S",
         IT1 !="DESCANSO")%>%
...
P31IsRaw<-RawDataSet1%>%
  bind_rows(RawDataSet2)

save(P31IsRaw, file = "P31IsRaw.RData")
