...
For most sighted people, touching a sign in braille feels like little more than a textured surface.  Similarly, when we see graphemes of an alphabet or a script that we are unfamiliar with, it is hard to distinguish among different letters that look "alike". Readers of braille (or any other script), on the other hand, automatically extract the relevant critical features in letters, and are for the most part unbothered by the accidental features of letters.   There is, indeed, a long tradition in the visual word recognition literature that has reliably demonstrated that the extraction of the relevant letter features to form abstract orthographic representations is quite robust to many manipulations. This article aims to understand what are the features of braille letters, and if such features change depending on braille literacy or if they emerge even in naive participants.


\pga{cite Pandemonium; Grainger et al 2008, etc etc.}


\pga{Explain what Braille is}

Braille is unique among writing systems for many reasons and one of them is that it is a modern invention (1824); unlike most other visual writing systems that have evolved through factors such as ergnomic constraints (cite something), braille is   \pga{blah, blah}.
